/**
 * Download (yt-dlp) + compress + import vidéos Kitesurf College.
 * Si la figure a déjà des vidéos → on AJOUTÉ (skip seulement si même titre).
 *
 * Usage :
 *   npx tsx scripts/import-kitesurfcollege-videos.ts --dry-run
 *   npx tsx scripts/import-kitesurfcollege-videos.ts
 *   npx tsx scripts/import-kitesurfcollege-videos.ts --skip-download
 *   npx tsx scripts/import-kitesurfcollege-videos.ts --skip-compress
 *   npx tsx scripts/import-kitesurfcollege-videos.ts --only=id1,id2
 *
 * Membres-only : exporter des cookies YouTube (compte membre) puis :
 *   YTDLP_COOKIES=/path/to/cookies.txt npm run import:kitesurfcollege
 *
...
import {
  KITESURF_COLLEGE_LESSONS,
  type KitesurfCollegeLesson,
} from "./data/kitesurfcollege-catalog";
import { compressUnderLimit } from "./lib/compress-under-limit";

const BUCKET = "figure-videos";
const RAW_DIR = path.join(process.cwd(), ".tmp", "kitesurfcollege-raw");
const OUT_DIR = path.join(process.cwd(), ".tmp", "kitesurfcollege-compressed");
...
function isMembersOnlyError(text: string): boolean {
  return /members-only|Join this channel to get access/i.test(text);
}

function download(lesson: KitesurfCollegeLesson, ytDlp: string): string {
  ...
  // Compte membre Kitesurf College (Netscape cookies.txt)
  if (cookies && existsSync(cookies)) {
    args.push("--cookies", cookies);
  }
  args.push(lesson.url);
  try {
    execFileSync(ytDlp, args, { stdio: ["ignore", "inherit", "pipe"] });
  } catch (e) {
    const err = e as { stderr?: Buffer; message?: string };
    const stderr = err.stderr?.toString("utf8") || err.message || "";
    if (isMembersOnlyError(stderr)) throw new Error("members-only");
    throw new Error(
...
async function main() {
  ...
  console.log(`\n${KITESURF_COLLEGE_LESSONS.length} leçons Kitesurf College\n`);
  if (DRY) {
    for (const l of KITESURF_COLLEGE_LESSONS) {
  ...
  for (const lesson of KITESURF_COLLEGE_LESSONS) {
