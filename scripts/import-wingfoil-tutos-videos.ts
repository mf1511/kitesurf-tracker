/**
 * Download (yt-dlp) + compress + import vidéos Tutos Wingfoil.
 * Si la figure a déjà des vidéos → on AJOUTÉ (skip seulement si même titre).
 *
 * Usage :
 *   npx tsx scripts/import-wingfoil-tutos-videos.ts --dry-run
 *   npx tsx scripts/import-wingfoil-tutos-videos.ts
 *   npx tsx scripts/import-wingfoil-tutos-videos.ts --skip-download
 *   npx tsx scripts/import-wingfoil-tutos-videos.ts --skip-compress
 *   npx tsx scripts/import-wingfoil-tutos-videos.ts --only=id1,id2
 *
...
import {
  WINGFOIL_TUTOS_LESSONS,
  type WingfoilTutosLesson,
} from "./data/wingfoil-tutos-catalog";
import { compressUnderLimit } from "./lib/compress-under-limit";

const BUCKET = "figure-videos";
const RAW_DIR = path.join(process.cwd(), ".tmp", "wingfoil-tutos-raw");
const OUT_DIR = path.join(process.cwd(), ".tmp", "wingfoil-tutos-compressed");
...
function download(lesson: WingfoilTutosLesson, ytDlp: string): string {
...
async function main() {
  ...
  console.log(`\n${WINGFOIL_TUTOS_LESSONS.length} leçons Tutos Wingfoil\n`);
  if (DRY) {
    for (const l of WINGFOIL_TUTOS_LESSONS) {
  ...
  for (const lesson of WINGFOIL_TUTOS_LESSONS) {
