/**
 * Download (yt-dlp) + compress + import vidéos Les Coachings de Pierre.
 * Si la figure a déjà des vidéos → on AJOUTÉ (skip seulement si même titre).
 *
 * Usage :
 *   npx tsx scripts/import-pierre-videos.ts --dry-run
 *   npx tsx scripts/import-pierre-videos.ts
 *   npx tsx scripts/import-pierre-videos.ts --skip-download
 *   npx tsx scripts/import-pierre-videos.ts --skip-compress
 *   npx tsx scripts/import-pierre-videos.ts --only=id1,id2
 *
...
import {
  PIERRE_LESSONS,
  type PierreLesson,
} from "./data/pierre-catalog";
import { compressUnderLimit } from "./lib/compress-under-limit";

const BUCKET = "figure-videos";
const RAW_DIR = path.join(process.cwd(), ".tmp", "pierre-raw");
const OUT_DIR = path.join(process.cwd(), ".tmp", "pierre-compressed");
...
function download(lesson: PierreLesson, ytDlp: string): string {
...
async function main() {
  ...
  console.log(`\n${PIERRE_LESSONS.length} leçons Les Coachings de Pierre\n`);
  if (DRY) {
    for (const l of PIERRE_LESSONS) {
  ...
  for (const lesson of PIERRE_LESSONS) {
    ...
    try {
      ...
      if (!figure && lesson.create) {
        // Sécurité / Tutoriels : inactive jusqu’à curation admin
