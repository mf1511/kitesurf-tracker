/**
 * Download (yt-dlp) + compress + import vidéos Duotone Academy Hooked.
 * Si la figure a déjà des vidéos → on AJOUTÉ (skip seulement si même titre).
 *
 * Usage :
 *   npx tsx scripts/import-duotone-hooked-videos.ts --dry-run
 *   npx tsx scripts/import-duotone-hooked-videos.ts
 *   npx tsx scripts/import-duotone-hooked-videos.ts --skip-download
 *   npx tsx scripts/import-duotone-hooked-videos.ts --skip-compress
 *   npx tsx scripts/import-duotone-hooked-videos.ts --only=id1,id2
 *
...
import {
  DUOTONE_HOOKED_LESSONS,
  type DuotoneHookedLesson,
} from "./data/duotone-hooked-catalog";
import { compressUnderLimit } from "./lib/compress-under-limit";

const BUCKET = "figure-videos";
const RAW_DIR = path.join(process.cwd(), ".tmp", "duotone-hooked-raw");
const OUT_DIR = path.join(process.cwd(), ".tmp", "duotone-hooked-compressed");
...
function download(lesson: DuotoneHookedLesson, ytDlp: string): string {
...
async function main() {
  ...
  console.log(`\n${DUOTONE_HOOKED_LESSONS.length} leçons Duotone Academy Hooked\n`);
  if (DRY) {
    for (const l of DUOTONE_HOOKED_LESSONS) {
  ...
  for (const lesson of DUOTONE_HOOKED_LESSONS) {
