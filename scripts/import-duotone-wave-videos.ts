/**
 * Download (yt-dlp) + compress + import vidéos Duotone Academy Wave.
 * Si la figure a déjà des vidéos → on AJOUTÉ (skip seulement si même titre).
 *
 * Usage :
 *   npx tsx scripts/import-duotone-wave-videos.ts --dry-run
 *   npx tsx scripts/import-duotone-wave-videos.ts
 *   npx tsx scripts/import-duotone-wave-videos.ts --skip-download
 *   npx tsx scripts/import-duotone-wave-videos.ts --skip-compress
 *   npx tsx scripts/import-duotone-wave-videos.ts --only=id1,id2
 *
...
import {
  DUOTONE_WAVE_LESSONS,
  type DuotoneWaveLesson,
} from "./data/duotone-wave-catalog";
import { compressUnderLimit } from "./lib/compress-under-limit";

const BUCKET = "figure-videos";
const RAW_DIR = path.join(process.cwd(), ".tmp", "duotone-wave-raw");
const OUT_DIR = path.join(process.cwd(), ".tmp", "duotone-wave-compressed");
...
function download(lesson: DuotoneWaveLesson, ytDlp: string): string {
...
async function main() {
  ...
  console.log(`\n${DUOTONE_WAVE_LESSONS.length} leçons Duotone Academy Wave\n`);
  if (DRY) {
    for (const l of DUOTONE_WAVE_LESSONS) {
  ...
  for (const lesson of DUOTONE_WAVE_LESSONS) {
