/**
 * Download (yt-dlp) + compress + import vidéos Duotone Academy Strapless.
 * Si la figure a déjà des vidéos → on AJOUTÉ (skip seulement si même titre).
 *
 * Usage :
 *   npx tsx scripts/import-duotone-strapless-videos.ts --dry-run
 *   npx tsx scripts/import-duotone-strapless-videos.ts
 *   npx tsx scripts/import-duotone-strapless-videos.ts --skip-download
 *   npx tsx scripts/import-duotone-strapless-videos.ts --skip-compress
 *   npx tsx scripts/import-duotone-strapless-videos.ts --only=id1,id2
 *
...
import {
  DUOTONE_STRAPLESS_LESSONS,
  type DuotoneStraplessLesson,
} from "./data/duotone-strapless-catalog";
import { compressUnderLimit } from "./lib/compress-under-limit";

const BUCKET = "figure-videos";
const RAW_DIR = path.join(process.cwd(), ".tmp", "duotone-strapless-raw");
const OUT_DIR = path.join(process.cwd(), ".tmp", "duotone-strapless-compressed");
...
function download(lesson: DuotoneStraplessLesson, ytDlp: string): string {
...
async function main() {
  ...
  console.log(`\n${DUOTONE_STRAPLESS_LESSONS.length} leçons Duotone Academy Strapless\n`);
  if (DRY) {
    for (const l of DUOTONE_STRAPLESS_LESSONS) {
  ...
  for (const lesson of DUOTONE_STRAPLESS_LESSONS) {
