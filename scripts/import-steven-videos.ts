/**
 * Download (yt-dlp) + compress + import vidéos Steven Akkersdijk.
 *
 * Usage :
 *   npx tsx scripts/import-steven-videos.ts --dry-run
 *   npx tsx scripts/import-steven-videos.ts
 *   npx tsx scripts/import-steven-videos.ts --skip-download   # raw déjà là
 *   npx tsx scripts/import-steven-videos.ts --skip-compress
 *   npx tsx scripts/import-steven-videos.ts --only=id1,id2
 *
...
import {
  STEVEN_LESSONS,
  type StevenLesson,
} from "./data/steven-akkersdijk-catalog";
import { compressUnderLimit } from "./lib/compress-under-limit";

const BUCKET = "figure-videos";
const RAW_DIR = path.join(process.cwd(), ".tmp", "steven-raw");
const OUT_DIR = path.join(process.cwd(), ".tmp", "steven-compressed");
...
function download(lesson: StevenLesson, ytDlp: string): string {
...
async function main() {
  ...
  console.log(`\n${STEVEN_LESSONS.length} leçons Steven Akkersdijk\n`);
  if (DRY) {
    for (const l of STEVEN_LESSONS) {
  ...
  for (const lesson of STEVEN_LESSONS) {
      ...
      if (!figure && lesson.create) {
        ...
        figure = await prisma.figure.create({
          data: {
            ...
            description:
              "Compilation Easy Tricks — SA Masterclass (Steven Akkersdijk).",
            steps: JSON.stringify([
              "Regarde la vidéo jusqu’au bout",
              "Repère 1–2 tricks à travailler",
        ...
        console.log(`  figure Bonus ${figure.slug}`);
      } else if (figure) {
        console.log(
