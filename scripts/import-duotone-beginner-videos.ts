/**
 * Download (yt-dlp) + compress + import vidéos Duotone Academy Beginner.
 * Si la figure a déjà des vidéos → on AJOUTÉ (skip seulement si même titre).
 *
 * Usage :
 *   npx tsx scripts/import-duotone-beginner-videos.ts --dry-run
 *   npx tsx scripts/import-duotone-beginner-videos.ts
 *   npx tsx scripts/import-duotone-beginner-videos.ts --skip-download
 *   npx tsx scripts/import-duotone-beginner-videos.ts --skip-compress
 *   npx tsx scripts/import-duotone-beginner-videos.ts --only=id1,id2
 *
...
import {
  DUOTONE_BEGINNER_LESSONS,
  type DuotoneBeginnerLesson,
} from "./data/duotone-beginner-catalog";
import { compressUnderLimit } from "./lib/compress-under-limit";

const BUCKET = "figure-videos";
const RAW_DIR = path.join(process.cwd(), ".tmp", "duotone-beginner-raw");
const OUT_DIR = path.join(process.cwd(), ".tmp", "duotone-beginner-compressed");
...
function download(lesson: DuotoneBeginnerLesson, ytDlp: string): string {
...
async function main() {
  ...
  console.log(`\n${DUOTONE_BEGINNER_LESSONS.length} leçons Duotone Academy Beginner\n`);
  if (DRY) {
    for (const l of DUOTONE_BEGINNER_LESSONS) {
  ...
  for (const lesson of DUOTONE_BEGINNER_LESSONS) {
