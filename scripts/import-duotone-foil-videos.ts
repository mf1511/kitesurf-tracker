/**
 * Download (yt-dlp) + compress + import vidéos Duotone Academy Foil.
 * Si la figure a déjà des vidéos → on AJOUTÉ (skip seulement si même titre).
 *
 * Usage :
 *   npx tsx scripts/import-duotone-foil-videos.ts --dry-run
 *   npx tsx scripts/import-duotone-foil-videos.ts
 *   npx tsx scripts/import-duotone-foil-videos.ts --skip-download
 *   npx tsx scripts/import-duotone-foil-videos.ts --skip-compress
 *   npx tsx scripts/import-duotone-foil-videos.ts --only=id1,id2
 *
...
import {
  DUOTONE_FOIL_LESSONS,
  type DuotoneFoilLesson,
} from "./data/duotone-foil-catalog";
import { compressUnderLimit } from "./lib/compress-under-limit";

const BUCKET = "figure-videos";
const RAW_DIR = path.join(process.cwd(), ".tmp", "duotone-foil-raw");
const OUT_DIR = path.join(process.cwd(), ".tmp", "duotone-foil-compressed");
...
function download(lesson: DuotoneFoilLesson, ytDlp: string): string {
...
async function main() {
  ...
  console.log(`\n${DUOTONE_FOIL_LESSONS.length} leçons Duotone Academy Foil\n`);
  if (DRY) {
    for (const l of DUOTONE_FOIL_LESSONS) {
  ...
  for (const lesson of DUOTONE_FOIL_LESSONS) {
