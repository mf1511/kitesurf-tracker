/**
 * Download (yt-dlp) + compress + import vidéos Tutos Kitesurf UCPA.
 * Si la figure a déjà des vidéos → on AJOUTÉ (skip seulement si même titre).
 *
 * Usage :
 *   npx tsx scripts/import-ucpa-kitesurf-videos.ts --dry-run
 *   npx tsx scripts/import-ucpa-kitesurf-videos.ts
 *   npx tsx scripts/import-ucpa-kitesurf-videos.ts --skip-download
 *   npx tsx scripts/import-ucpa-kitesurf-videos.ts --skip-compress
 *   npx tsx scripts/import-ucpa-kitesurf-videos.ts --only=id1,id2
 *
...
import {
  UCPA_KITESURF_LESSONS,
  type UcpaKitesurfLesson,
} from "./data/ucpa-kitesurf-catalog";
import { compressUnderLimit } from "./lib/compress-under-limit";

const BUCKET = "figure-videos";
const RAW_DIR = path.join(process.cwd(), ".tmp", "ucpa-kitesurf-raw");
const OUT_DIR = path.join(process.cwd(), ".tmp", "ucpa-kitesurf-compressed");
...
function download(lesson: UcpaKitesurfLesson, ytDlp: string): string {
...
async function main() {
  ...
  console.log(`\n${UCPA_KITESURF_LESSONS.length} leçons Tutos Kitesurf UCPA\n`);
  if (DRY) {
    for (const l of UCPA_KITESURF_LESSONS) {
  ...
  for (const lesson of UCPA_KITESURF_LESSONS) {
